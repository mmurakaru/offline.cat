...
export function OutlineSidebar({
  ...
  const groups =
    slideGroups.length > 0
      ? slideGroups
      : [{ slideIndex: 0, segments }];

  const groupLabel = fileType === "pptx" ? "Slide" : "Page";

  return (
    <div className="h-full overflow-y-auto bg-gray-50 dark:bg-gray-950 border-r dark:border-gray-800">
      <div className="p-2">
        {groups.map((group) => (
          <div key={group.slideIndex} className="mb-3">
            ...
            {group.segments.map((segment) => (
              <button
                ...
                className={`w-full text-left px-2 py-1.5 rounded text-xs cursor-pointer transition-colors flex items-start gap-2 ${
              ...
              >
                <span
                  className={`mt-1 w-1.5 h-1.5 rounded-full shrink-0 ${getStatusColor(segment)}`}
                />
